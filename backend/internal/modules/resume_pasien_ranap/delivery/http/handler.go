...
import (
	...
	modul "simrs-backend/internal/modules/resume_pasien_ranap"
...
func (h *Handler) simpan(c *gin.Context, ubah bool) {
	...
	if c.ShouldBindJSON(&input) != nil {
		httpresponse.Error(c, http.StatusUnprocessableEntity, "RESUME_PASIEN_RANAP_VALIDATION_ERROR", "Data resume pasien rawat inap tidak lengkap")
		return
	}
	var err error
	if ubah {
	...
	status, pesan := http.StatusCreated, "Resume pasien rawat inap berhasil disimpan"
	if ubah {
		status, pesan = http.StatusOK, "Resume pasien rawat inap berhasil diperbarui"
	}
...
func (h *Handler) Hapus(c *gin.Context) {
	...
	httpresponse.Success(c, http.StatusOK, gin.H{"pesan": "Resume pasien rawat inap berhasil dihapus"})
}

func (h *Handler) error(c *gin.Context, err error) {
	log.Printf("resume pasien rawat inap gagal diproses: %v", err)
	switch {
	case errors.Is(err, modul.ErrInputTidakValid):
		httpresponse.Error(c, http.StatusUnprocessableEntity, "RESUME_PASIEN_RANAP_VALIDATION_ERROR", err.Error())
	case errors.Is(err, modul.ErrSudahAda):
		httpresponse.Error(c, http.StatusConflict, "RESUME_PASIEN_RANAP_DUPLICATE", err.Error())
	case errors.Is(err, modul.ErrBillingTerkunci):
		httpresponse.Error(c, http.StatusConflict, "RESUME_PASIEN_RANAP_BILLING_LOCKED", "Billing sudah terverifikasi atau kunjungan dibatalkan. Resume tidak dapat ditambah, diubah, atau dihapus.")
	case errors.Is(err, modul.ErrTidakDitemukan):
		httpresponse.Error(c, http.StatusNotFound, "RESUME_PASIEN_RANAP_NOT_FOUND", err.Error())
	default:
		httpresponse.Error(c, http.StatusServiceUnavailable, "RESUME_PASIEN_RANAP_SIMRS_UNAVAILABLE", "Resume pasien rawat inap tidak dapat diproses pada SIMRS Khanza")
	}
}
