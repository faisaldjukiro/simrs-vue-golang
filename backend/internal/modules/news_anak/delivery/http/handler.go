...
type Handler struct {
	repo *news_anak.Repositori
}

func NewHandler(repo *news_anak.Repositori) *Handler { return &Handler{repo: repo} }
...
func (h *Handler) proses(c *gin.Context) {
	...
	var in news_anak.Input
	if c.ShouldBindJSON(&in) != nil {
		tulisError(c, news_anak.ErrValidasi)
		return
	}
...
func tulisError(c *gin.Context, err error) {
	...
	case errors.Is(err, news_anak.ErrAkses):
		status, kode, pesan = 403, "FORBIDDEN", err.Error()
	case errors.Is(err, news_anak.ErrKonflik):
