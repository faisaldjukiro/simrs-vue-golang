...
func defaultInput(noRawat string) Input {
	return Input{
		...
		Kepala: "Normal", Mata: "Normal", Gigi: "Normal", Leher: "Normal", Thoraks: "Normal",
		Abdomen: "Normal", Genital: "Normal", Ekstremitas: "Normal", 
	}
}
...
func (r *Repositori) ambil(ctx context.Context, noRawat string) (Input, Dokter, error) {
	...
	err := r.simrsDB.QueryRowContext(ctx, `
		SELECT p.no_rawat, DATE_FORMAT(p.tanggal,'%Y-%m-%d %H:%i:%s'), p.kd_dokter,
			...
			p.kepala,p.mata,p.gigi,p.leher,p.thoraks,p.abdomen,p.ekstremitas,
			p.genital,p.ket_fisik,p.ket_lokalis,p.ekg,p.rad,p.lab,p.diagnosis,p.tata,
	...
	if err != nil && !errors.Is(err, sql.ErrNoRows) {
		return Input{}, Dokter{}, fmt.Errorf("baca penilaian Awal Medis IGD: %w", err)
	}
...
func (r *Repositori) Simpan(ctx context.Context, input Input, ubah bool) error {
	return r.dalamTransaksi(ctx, input.NoRawat, func(tx *sql.Tx) error {
		if ubah {
			hasil, err := tx.ExecContext(ctx, `UPDATE penilaian_medis_igd SET tanggal=?,kd_dokter=?,anamnesis=?,hubungan=?,keluhan_utama=?,rps=?,rpd=?,rpk=?,rpo=?,alergi=?,keadaan=?,gcs=?,kesadaran=?,td=?,nadi=?,rr=?,suhu=?,spo=?,bb=?,tb=?,kepala=?,mata=?,gigi=?,leher=?,thoraks=?,abdomen=?,ekstremitas=?,genital=?,ket_fisik=?,ket_lokalis=?,ekg=?,rad=?,lab=?,diagnosis=?,tata=? WHERE no_rawat=?`, nilaiSimpan(input, false)...)
			if err != nil {
				return fmt.Errorf("ubah penilaian Awal Medis IGD: %w", err)
			}
			...
			if jumlah == 0 {
				var ada int
				if err := tx.QueryRowContext(ctx, `SELECT COUNT(*) FROM penilaian_medis_igd WHERE no_rawat=?`, input.NoRawat).Scan(&ada); err != nil {
					return err
				}
				if ada == 0 {
		...
		_, err := tx.ExecContext(ctx, `INSERT INTO penilaian_medis_igd (no_rawat,tanggal,kd_dokter,anamnesis,hubungan,keluhan_utama,rps,rpd,rpk,rpo,alergi,keadaan,gcs,kesadaran,td,nadi,rr,suhu,spo,bb,tb,kepala,mata,gigi,leher,thoraks,abdomen,ekstremitas,genital,ket_fisik,ket_lokalis,ekg,rad,lab,diagnosis,tata) VALUES (?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?)`, nilaiSimpan(input, true)...)
		var mysqlErr *mysql.MySQLError
		if errors.As(err, &mysqlErr) && mysqlErr.Number == 1062 {
			return ErrSudahAda
		}
		if err != nil {
			return fmt.Errorf("simpan penilaian Awal Medis IGD: %w", err)
		}
		return nil
	})
}
...
func (r *Repositori) Hapus(ctx context.Context, noRawat string) error {
	return r.dalamTransaksi(ctx, noRawat, func(tx *sql.Tx) error {
		hasil, err := tx.ExecContext(ctx, `DELETE FROM penilaian_medis_igd WHERE no_rawat=?`, noRawat)
		if err != nil {
			return fmt.Errorf("hapus penilaian Awal Medis IGD: %w", err)
		}
		jumlah, err := hasil.RowsAffected()
		if err != nil {
			return err
...
func nilaiSimpan(i Input, sertakanNoRawat bool) []any {
	nilai := []any{i.Tanggal, i.KodeDokter, i.Anamnesis, i.Hubungan, i.KeluhanUtama, i.RPS, i.RPD, i.RPK, i.RPO, i.Alergi, i.Keadaan, i.GCS, i.Kesadaran, i.TD, i.Nadi, i.RR, i.Suhu, i.SPO2, i.BB, i.TB, i.Kepala, i.Mata, i.Gigi, i.Leher, i.Thoraks, i.Abdomen, i.Ekstremitas, i.Genital, i.KetFisik, i.KetLokalis, i.EKG, i.Rad, i.Lab, i.Diagnosis, i.Tata}
	if sertakanNoRawat {
