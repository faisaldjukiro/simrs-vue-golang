...
type Input struct {
	...
	KonsulRujuk  string `json:"konsulrujuk"`
}
...
func defaultInput(noRawat string) Input {
	return Input{
		...
		Kepala: "Normal", Gigi: "Normal", THT: "Normal", Thoraks: "Normal",
		Abdomen: "Normal", Genital: "Normal", Ekstremitas: "Normal", Kulit: "Normal",
	}
}
...
func (r *Repositori) ambil(ctx context.Context, noRawat string) (Input, Dokter, error) {
	...
	err := r.simrsDB.QueryRowContext(ctx, `
		SELECT p.no_rawat, DATE_FORMAT(p.tanggal,'%Y-%m-%d %H:%i:%s'), p.kd_dokter,
			...
			p.kepala,p.gigi,p.tht,p.thoraks,p.abdomen,p.genital,p.ekstremitas,p.kulit,
			p.ket_fisik,p.ket_lokalis,p.penunjang,p.diagnosis,p.tata,p.konsulrujuk,
	...
	if err != nil && !errors.Is(err, sql.ErrNoRows) {
		return Input{}, Dokter{}, fmt.Errorf("baca penilaian awal medis umum: %w", err)
	}
...
func (r *Repositori) Simpan(ctx context.Context, input Input, ubah bool) error {
	return r.dalamTransaksi(ctx, input.NoRawat, func(tx *sql.Tx) error {
		if ubah {
			hasil, err := tx.ExecContext(ctx, `UPDATE penilaian_medis_ralan SET tanggal=?,kd_dokter=?,anamnesis=?,hubungan=?,keluhan_utama=?,rps=?,rpd=?,rpk=?,rpo=?,alergi=?,keadaan=?,gcs=?,kesadaran=?,td=?,nadi=?,rr=?,suhu=?,spo=?,bb=?,tb=?,kepala=?,gigi=?,tht=?,thoraks=?,abdomen=?,genital=?,ekstremitas=?,kulit=?,ket_fisik=?,ket_lokalis=?,penunjang=?,diagnosis=?,tata=?,konsulrujuk=? WHERE no_rawat=?`, nilaiSimpan(input, false)...)
			if err != nil {
				return fmt.Errorf("ubah penilaian awal medis umum: %w", err)
			}
			...
			if jumlah == 0 {
				var ada int
				if err := tx.QueryRowContext(ctx, `SELECT COUNT(*) FROM penilaian_medis_ralan WHERE no_rawat=?`, input.NoRawat).Scan(&ada); err != nil {
					return err
				}
				if ada == 0 {
		...
		_, err := tx.ExecContext(ctx, `INSERT INTO penilaian_medis_ralan (no_rawat,tanggal,kd_dokter,anamnesis,hubungan,keluhan_utama,rps,rpd,rpk,rpo,alergi,keadaan,gcs,kesadaran,td,nadi,rr,suhu,spo,bb,tb,kepala,gigi,tht,thoraks,abdomen,genital,ekstremitas,kulit,ket_fisik,ket_lokalis,penunjang,diagnosis,tata,konsulrujuk) VALUES (?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?)`, nilaiSimpan(input, true)...)
		var mysqlErr *mysql.MySQLError
		if errors.As(err, &mysqlErr) && mysqlErr.Number == 1062 {
			return ErrSudahAda
		}
		if err != nil {
			return fmt.Errorf("simpan penilaian awal medis umum: %w", err)
		}
		return nil
	})
}
...
func (r *Repositori) Hapus(ctx context.Context, noRawat string) error {
	return r.dalamTransaksi(ctx, noRawat, func(tx *sql.Tx) error {
		hasil, err := tx.ExecContext(ctx, `DELETE FROM penilaian_medis_ralan WHERE no_rawat=?`, noRawat)
		if err != nil {
			return fmt.Errorf("hapus penilaian awal medis umum: %w", err)
		}
		jumlah, err := hasil.RowsAffected()
		if err != nil {
			return err
...
func nilaiSimpan(i Input, sertakanNoRawat bool) []any {
	nilai := []any{i.Tanggal, i.KodeDokter, i.Anamnesis, i.Hubungan, i.KeluhanUtama, i.RPS, i.RPD, i.RPK, i.RPO, i.Alergi, i.Keadaan, i.GCS, i.Kesadaran, i.TD, i.Nadi, i.RR, i.Suhu, i.SPO2, i.BB, i.TB, i.Kepala, i.Gigi, i.THT, i.Thoraks, i.Abdomen, i.Genital, i.Ekstremitas, i.Kulit, i.KetFisik, i.KetLokalis, i.Penunjang, i.Diagnosis, i.Tata, i.KonsulRujuk}
	if sertakanNoRawat {
