...
LEFT JOIN sidebar_pasien pemilik
    ON pemilik.kode_sidebar = 'lanjutan_risiko_jatuh_anak'
    AND pemilik.id <> target.id
SET target.kode_sidebar = 'lanjutan_risiko_jatuh_anak',
    target.updated_at = NOW()
WHERE target.nama_sidebar = 'Lanjutan Risiko Jatuh Anak'
...
SELECT 'lanjutan_risiko_jatuh_anak', 'Lanjutan Risiko Jatuh Anak',
    'TriangleAlert', JSON_ARRAY('Rawat Inap'), 40, TRUE, NOW(), NOW()
WHERE NOT EXISTS (
    SELECT 1 FROM sidebar_pasien WHERE kode_sidebar = 'lanjutan_risiko_jatuh_anak'
);
