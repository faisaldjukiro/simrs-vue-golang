...
LEFT JOIN sidebar_pasien pemilik
    ON pemilik.kode_sidebar = 'pews_anak'
    AND pemilik.id <> target.id
LEFT JOIN sidebar_pasien kandidat
    ON kandidat.nama_sidebar IN ('Pemantauan PEWS Anak', 'PEWS Anak')
    AND kandidat.id < target.id
SET target.kode_sidebar = 'pews_anak', target.updated_at = NOW()
WHERE target.nama_sidebar IN ('Pemantauan PEWS Anak', 'PEWS Anak')
