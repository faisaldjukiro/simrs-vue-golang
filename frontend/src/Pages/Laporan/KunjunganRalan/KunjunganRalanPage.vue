  ...
  <section class="visit-report-module">
    <header class="visit-report-header">
      <div>
        ...
        <p>Adaptasi laporan DlgKunjunganRalan SIMRS Khanza.</p>
      </div>
    ...
    <nav class="visit-report-tabs">
      <button
        ...
        :key="tab.v"
        :class="{ active: filter.jenis === tab.v }"
        @click="gantiJenis(tab.v)"
      >
        {{ tab.l }}
      </button>
    </nav>
    ...
    <div class="visit-report-summary">
      ...
      ><template v-else>
        ...
        <article>
          ...
          ><strong>{{ ringkasan.tidak_berulang }}</strong>
        </article>
      </template>
      <article>
    ...
    <div class="visit-report-tools">
      <span>{{ data.length }} baris ditampilkan</span
      ><button
        class="chart-button"
      ...
      <div class="visit-chart-grid">
        <article v-if="!berulang" class="visit-chart-card trend-chart">
          ...
          <div v-if="trenHarian.length" class="vertical-chart">
            <div v-for="item in trenHarian" :key="item.label">
              <span
                ><i :style="{ height: `${Math.max(item.persen, 3)}%` }"></i
                ><b>{{ item.nilai }}</b></span
        ...
        <article class="visit-chart-card donut-card">
          ...
          <div class="donut-content">
            <i class="donut gender" :style="{ '--value': `${persenGender}%` }"
              ><b>{{ persenGender }}%</b><small>Laki-laki</small></i
            >
            <ul>
              <li>
        ...
        <article v-if="!berulang" class="visit-chart-card donut-card">
          ...
          <div class="donut-content">
            <i class="donut status" :style="{ '--value': `${persenBaru}%` }"
              ><b>{{ persenBaru }}%</b><small>Pasien baru</small></i
            >
            <ul>
              <li>
    ...
    <DataTable
      ...
      :rows="data"
      :data-key="berulang ? 'no_rm' : 'no_rawat'"
      :loading="loading"
      paginator
      :rows-per-page="25"
      :rows-per-page-options="[25, 50, 100]"
      empty-message="Data kunjungan tidak ditemukan."
      @page="gantiHalaman"
    >
      <Column
        header="No."
        ...
        ><template #body="{ index }"
          ><strong>{{ firstRow + index + 1 }}</strong></template
        ></Column
      >
      <template v-if="!berulang">
        <Column header="Tanggal / No. Rawat" style="min-width: 165px"
          ><template #body="{ data: r }"
            ><strong>{{ r.tanggal }} {{ r.jam }}</strong
            ><small>{{ r.no_rawat }}</small></template
          >
        </Column>
        <Column header="RM / Pasien" style="min-width: 220px"
          ><template #body="{ data: r }"
            ><strong>{{ r.nama_pasien }}</strong
            ><small>{{ r.no_rm }} · {{ r.status_daftar }}</small></template
          >
        </Column>
        <Column header="Laki-laki" style="min-width: 95px"
          ><template #body="{ data: r }"
            ><strong>{{
              r.jenis_kelamin === "L"
                ? `L
          ...
          ></Column
        >
        <Column header="Perempuan" style="min-width: 95px"
          ><template #body="{ data: r }"
            ><strong>{{
              r.jenis_kelamin === "P"
                ? `P
        ...
        <Column header="Diagnosis" style="min-width: 320px"
          ><template #body="{ data: r }"
            ><strong>{{ r.kode_diagnosa || "-" }}</strong
            ><small>{{ r.diagnosa || "-" }}</small></template
          >
        </Column>
        <Column header="Pelayanan" style="min-width: 240px"
          ><template #body="{ data: r }"
            ><strong>{{ r.poli }}</strong
            ><small>{{ r.dokter }}</small></template
          ></Column
        >
        <Column header="Penjamin / SEP" style="min-width: 210px"
          ><template #body="{ data: r }"
            ><strong>{{ r.penjamin }}</strong
            ><small>{{ r.no_sep || "-" }}</small></template
          ></Column
        >
      </template>
      <template v-else>
