...
import type { PropsRisikoJatuh } from '../../../types/risikoJatuhAnak'
import { useRisikoJatuhAnak } from './useRisikoJatuhAnak'
  ...
  <section class="risiko-jatuh-page clinical-page">
    <article class="clinical-form-card">
      <header class="clinical-section-header">
        <div>
          ...
          <h3>{{ editing ? 'Edit Penilaian' : 'Lanjutan Risiko Jatuh Anak' }}</h3>
          <p>Penilaian Humpty Dumpty mengikuti SIMRS Khanza. Penyimpanan langsung ke Khanza.</p>
        </div>
        ...
        <fieldset class="form-compact risiko-fields" :disabled="saving">
          ...
          <div class="risiko-grid">
            <div v-for="(item, i) in skala" :key="item.label" class="risiko-pertanyaan"
                :class="{ 'risiko-pertanyaan-panjang': i === 6 }">
              <FormInput
                v-model="form['penilaian_humptydumpty_skala' + (i + 1)]"
              ...
              <p v-if="form['penilaian_humptydumpty_skala' + (i + 1)]" class="risiko-pilihan-lengkap">{{ form['penilaian_humptydumpty_skala' + (i + 1)] }}</p>
            </div>
          </div>
          <section class="risiko-ringkasan" aria-label="Ringkasan penilaian Humpty Dumpty" aria-live="polite" aria-atomic="true">
            <div class="risiko-total">
              ...
              <div class="risiko-keterangan">
                ...
                <small>{{ total === null ? 'Lengkapi tujuh komponen penilaian' : 'Dihitung dari tujuh komponen penilaian' }}</small>
              </div>
            </div>
            <div class="risiko-status">
              ...
              <strong
                class="risiko-badge"
                :class="{ tinggi: total !== null && total >= 12, rendah: total !== null && total < 12 }"
              >
            ...
            <div class="risiko-panduan">
              ...
              <dl>
                <div :class="{ aktif: total !== null && total < 12 }">
                  <dt>Rendah</dt>
                  <dd>7–11</dd>
                </div>
                <div :class="{ aktif: total !== null && total >= 12 }">
    ...
    <article class="clinical-history-card">
      ...
      >
        ...
        <Column field="data.penilaian_humptydumpty_totalnilai" header="Skor" header-class="right-align-header" body-class="right-align-body" />
    ...
    <Dialog :visible="!!detail" modal header="Detail Penilaian Humpty Dumpty" :style="{ width: '720px', maxWidth: '95vw' }" @update:visible="detail = null">
      <div v-if="detail" class="risiko-detail">
        ...
        <dl>
          <template v-for="(item, i) in skala" :key="item.label">
            <dt>{{ item.label }}</dt>
            <dd>{{ detail.data['penilaian_humptydumpty_skala' + (i + 1)] }} — Skor {{ detail.data['penilaian_humptydumpty_nilai' + (i + 1)] }}</dd>
          </template>
          <dt>Total / Risiko</dt><dd>{{ detail.data.penilaian_humptydumpty_totalnilai }} / {{ detail.risiko }}</dd>
...
<style src="@/Pages/Pasien/RisikoJatuhAnak/risiko-jatuh-anak.css" scoped></style>
