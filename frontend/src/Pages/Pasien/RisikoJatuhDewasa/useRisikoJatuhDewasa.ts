...
import type { CatatanRisikoJatuh, HasilRisikoJatuh, PropsRisikoJatuh, SkalaMorse } from '../../../types/risikoJatuhDewasa'

export function useRisikoJatuhDewasa(props: PropsRisikoJatuh) {
  ...
  const nilai = computed(() => skala.value.map((s, i) => {
    const index = s.pilihan.indexOf(form['penilaian_jatuhmorse_skala' + (i + 1)])
    return index < 0 ? null : s.nilai[index]
  }))
  const total = computed(() => nilai.value.length === 6 && nilai.value.every(n => n != null)
    ? nilai.value.reduce<number>((jumlah, n) => jumlah + (n ?? 0), 0)
    : null)
  const risiko = computed(() => total.value === null ? 'Lengkapi enam penilaian'
    : total.value < 25 ? 'Rendah' : total.value < 45 ? 'Sedang' : 'Tinggi')
  ...
  const terkunci = computed(() => loading.value || saving.value || !!error.value || skala.value.length !== 6)

  function api<T>(path = '', options: RequestInit = {}) {
    return request<T>('/api/risiko-jatuh-dewasa' + path, {
      ...options,
  ...
  function reset() {
    ...
    for (let i = 1; i <= 6; i++) form['penilaian_jatuhmorse_skala' + i] = ''
    form.hasil_skrining = ''
    form.saran = ''
    waktuSekarang()
  }
  ...
  async function mutasi(hapus = false) {
    ...
    if (!hapus && (total.value === null || !petugas.value.kode || !form.tanggal || !form.hasil_skrining.trim() || !form.saran.trim())) {
      errorSimpan.value = 'Lengkapi tanggal, petugas, enam penilaian, hasil skrining, dan saran.'
      return
    }
    const konteks = generasi
    saving.value = true
